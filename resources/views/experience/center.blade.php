...
<div class="experience-page">
  ...
  <section id="rankings" class="feature-section"><div class="section-heading"><div><span>07</span><h2>多维排行榜</h2></div><p>总进度、每日活跃和邀请贡献</p></div><div class="ranking-columns">@foreach(['progress'=>'总进度榜','daily'=>'今日活跃榜','invite'=>'邀请榜'] as $key=>$label)<div><h3>{{ $label }}</h3><ol>@forelse($rankings[$key] as $rank=>$player)<li><em>{{ $rank+1 }}</em><span>{{ mb_substr($player->name,0,1) }}***</span><b>{{ $key==='progress'?$player->completed_laps.'圈 '.$player->current_position.'格':$player->score }}</b></li>@empty<li>暂无数据</li>@endforelse</ol></div>@endforeach</div></section>
