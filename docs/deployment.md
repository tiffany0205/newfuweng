...
升级到 `2026_07_14_000500_sync_landmarks_and_move_positions` 后，迁移会将已有活动的 12 个正式地标同步到旧 SQLite 数据库，并从历史走棋记录补回漏记的地标访问和可确定的幸运值，同时把旧“跳棋消耗”备注改为实际点数与第 1～36 格位置。迁移不会追发无法可靠还原的彩虹随机奖励或消耗型效果。执行前必须按第 10 节备份数据库。
